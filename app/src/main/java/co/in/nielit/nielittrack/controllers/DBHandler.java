...
public class DBHandler extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        try {
            ...
            Log.e("DBHandler", "Error occurred", e);  // Logs error details to Logcat
            Toast.makeText(context.getApplicationContext(), "Something went wrong: " + e.getMessage(), Toast.LENGTH_LONG).show(); // Shows user-friendly message
        }
    }
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        try {
            ...
            Log.e("DBHandler", "Exception occurred: " + e.getMessage(), e);
    ...
    public int addAttendanceSession(AttendanceSession attendanceSession) {
        ...
        try {
            ...
            String checkDuplicate = "SELECT attendance_session_id FROM attendance_session_table WHERE " +
                    "attendance_session_date = ? AND attendance_session_branch = ? AND " +
                    "attendance_session_sem = ? AND attendance_session_subject = ?";
            Cursor cursor = db.rawQuery(checkDuplicate, new String[]{
            ...
            if (cursor.moveToFirst()) {
                ...
                // Insert New Session
                String insertQuery = "INSERT INTO attendance_session_table (attendance_session_faculty_id, " +
                ...
                String getMaxIdQuery = "SELECT MAX(attendance_session_id) FROM attendance_session_table";
                Cursor maxCursor = db.rawQuery(getMaxIdQuery, null);
                if (maxCursor.moveToFirst()) {
                    sessionId = maxCursor.getInt(0);
                }
                maxCursor.close();
            }
            cursor.close();
    ...
    public ArrayList<AttendanceSession> getAttendanceSession() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                session.setAttendance_session_id(cursor.getInt(0));
                session.setAttendance_session_faculty_id(cursor.getInt(1));
                session.setAttendance_session_branch(cursor.getString(2));
                session.setAttendance_session_sem(cursor.getString(3));
                session.setAttendance_session_date(cursor.getString(4));
                session.setAttendance_session_subject(cursor.getString(5));
                list.add(session);
            } while (cursor.moveToNext());
        }
    ...
    public ArrayList<Attendance> getAttendanceBySessionID(AttendanceSession attendanceSession) {
        ...
        String query = "SELECT * FROM attendance_session_table WHERE " +
                ...
                + attendanceSession.getAttendance_session_faculty_id() + " AND attendance_session_branch='"
                + attendanceSession.getAttendance_session_branch()
